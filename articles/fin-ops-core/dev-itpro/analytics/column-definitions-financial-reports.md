...
1. Pārskatu veidotājā navigācijas rūtī noklikšķiniet uz Kolonnu **definīcijas**.
2. Izvēlnē Fails **noklikšķiniet** uz Jauns **un** pēc tam noklikšķiniet uz Kolonnas **definīcija**.
...
1. Pārskatu veidotājā noklikšķiniet uz **Kolonnu definīcijas** un pēc tam atveriet modificējamās kolonnas definīciju.
2. Atlasiet kolonnu, kur ir jāievieto jauna kolonna.
3. Izvēlnē Rediģēt **noklikšķiniet** uz kolonnas **Ievietot**. Jaunā kolonna tiek parādīta pa kreisi no jūsu atlasītās kolonnas.

### <a name="delete-a-column-from-a-column-definition"></a>Kolonnas dzēšana no kolonnas definīcijas

1. Pārskatu veidotājā noklikšķiniet uz **Kolonnu definīcijas** un pēc tam atveriet modificējamās kolonnas definīciju.
2. Atlasiet dzēšamo kolonnu.
3. Izvēlnē Rediģēt **noklikšķiniet** uz kolonnas **Dzēst**.
...
- Kolonnas definīcijas galveņu apgabalā ir galvenes teksts un pārskatā redzamais formatējums. Galveni var attiekties uz vienu datu kolonnu, var aptvert vairākas kolonnas vai var attiekties uz kolonnām pēc nosacījuma. Kolonnas definīcijā var būt tik kolonnu galveņu rindu, cik nepieciešams.
    ...
    > [!NOTE]
    > Kolonnu galvenes attiecas uz katru pārskata datu kolonnu. Pārskatu galvenes attiecas uz visu pārskatu. Pārskata galvenes tiek definētas pārskata **definīcijas cilnē Galvenes** un kājenes.
...
> [!NOTE]
> Kolonnas **ierobežojuma kods** ignorē visus konfliktējošos iestatījumus, kas ir piešķirti rindas definīcijā.

### <a name="column-restrictions-cell"></a>Kolonnas ierobežojumu šūna

Kolonnu **ierobežojumu** šūna var ietvert kodus, kas ierobežo vai nemāk informāciju, piemēram, rindu formatēšanu, detalizētu informāciju un summas.

#### <a name="add-a-column-restriction-in-a-column-definition"></a>Pievienot kolonnas ierobežojumu kolonnas definīcijai

1. Pārskatu veidotājā atveriet modificējamās kolonnas definīciju.
2. Veiciet dubultklikšķi uz kolonnas ierobežojumu **šūnas**, lai ierobežotu.
3. Dialoglodziņā Kolonnu **ierobežojumi** atlasiet sarakstā vienu vai vairākus kodus un pēc tam noklikšķiniet uz **Labi**.
...
1. Pārskatu veidotājā atveriet modificējamās kolonnas definīciju.
2. Veiciet dubultklikšķi uz kolonnas **pārskata vienības** šūnas, lai ierobežotu.
3.  **Pārskata vienības atlases** dialoglodziņā pārskata koka **sarakstā** atlasiet koku.
...
Varat pievienot, modificēt un dzēst pārskata galvenes, kas tiek rādītas kolonnu augšdaļā. Varat konfigurēt arī nosacījuma plānošanas kolonnu virsrakstus, **·**  **pamatojoties uz kolonnu definīciju lauku Periods un pārskata definīciju** lauku Bāzes periods. Pamata perioda līdzeklis palīdz ietaupīt laiku, veidojot apkopojumu prognožu pārskatus.

### <a name="create-and-manage-column-headers"></a>Kolonnu galveņu izveide un pārvaldība

Lai pievienotu, modificētu **un** dzēstu virsrakstus, kas parādīsies pārskata kolonnu augšā, var izmantot dialoglodziņu Kolonnas virsraksts. Tabulā ir aprakstīti lauki kolonnas **virsraksta** dialoglodziņā.

| Lauks                 | Apraksts |
|-----------------------|-------------|
| Kolonnas galvenes teksts    | Šis teksts tiek rādīts kolonnas galvenē. Tekstu šajā laukā var ievadīt tieši, vai noklikšķiniet uz Ievietot automātisko tekstu **, lai atlasītu opciju, kas atjaunina kolonnas virsrakstu katru reizi,** kad pārskats tiek ģenerēts. Lai iekļautu vairākus automātiskā teksta kodus, atkārtoti **noklikšķiniet uz** Ievietot automātisko tekstu un pēc tam sarakstā noklikšķiniet uz cita koda. |
...
3.  **Dialoglodziņā Kolonnas virsraksts** ievadiet kolonnas virsraksta tekstu. Alternatīvi noklikšķiniet **uz Ievietot automātisko** tekstu un atlasiet opciju.
...
3. Izvēlnē Rediģēt **noklikšķiniet** uz Iespraust **rindu**. Jaunā rinda tiek ievietota virs rindas, ko atlasījāt 2. darbībā.
...
- Tiek ignorēts jebkāds **apturēšanas nosacījums (** sadalīt laukā), kas ir saskaņots pirms sākuma nosacījuma (**izplatīšana** no lauka). Piemēram, kolonnai B sadalīšanas nosacījums ir definēts kā PAMATA+1 līdz PAMATA un vienums PAMATA ir norādīts kolonnā C, bet vienums PAMATA+1 — kolonnā D. Šādā gadījumā apturēšanas nosacījums kolonnā C tiek ignorēts, un galveņu drukāšana tiek sākta kolonnā D.
- Ja norādītās kolonnu galvenes pārklājas, to pārklājums būs redzams, drukājot pārskatu. Pārskats ir ģenerēts, bet **pārskata** rindas statusa laukā parādās šāds brīdinājums: "Kolonnu virsraksti, kas izmanto pamata pārklāšanos ar citu kolonnu virsrakstiem un var izraisīt teksta pārklāšanos." Piemēram, kolonnas B galvenes definīcija ir B līdz PAMATA+1, un kolonnas D galvenes definīcija ir PAMATA+1 līdz F. Šajā gadījumā galvenes tiek drukātas viena uz otras un nav salasāmas. Ikreiz, kad definīcijā **Sadalīt no/Sadalīt uz** tiek izmantota vērtība PAMATA, noteikti skatiet ģenerēto pārskatu, lai redzētu, vai galvenes nepārklājas.
...
Lietotājs divreiz noklikšķina uz kolonnas virsraksta šūnas B kolonnā, lai **atvērtu** kolonnas virsraksta dialoglodziņu, un ievada šādu informāciju.
...
Pēc informācijas ievadīšanas lietotājs noklikšķina uz **Labi**. Lietotājs tad veiciet dubultklikšķi uz kolonnas virsraksta šūnas C **kolonnā**, lai atvērtu kolonnas virsraksta dialoglodziņu, un ievada šādu informāciju.
...
Kad norādāt kolonnas definīciju, formatēšanas kolonnas detalizētās rindas tiek lietotas, lai atlasītajās kolonnās ieviestu īpašu formatējumu. Kaut arī dažas opcijas **Drukas kontroles** un **Kolonnu** ierobežojumu opcijas **ir specifiskas FD** kolonnām, lielākā daļa opciju attiecas uz visiem kolonnu tipiem. Kolonnas definīcijā norādītais formatējums ignorē pārskata definīcijā norādīto formatējumu. Tomēr rindas definīcijā norādītais formatējums ignorē kolonnas definīcijā norādīto formatējumu. Tālāk redzamās rindas tiek uzskatītas par formatēšanas rindām.
...
Kolonnas **platuma** šūna norāda rakstzīmju skaitu, kas jālieto šīs kolonnas platumam izdrukātajā pārskatā. Kolonnas platums ir svarīgs kolonnām, kas satur summas ( **CALC**, **WKS** vai **FD** tipa kolonnas), aprakstus ( **DESC** tipa kolonnas), vai aizpildījumu ( **FILL** tipa kolonnas). Pēc noklusējuma tiek atlasīta opcija **Automātiskā ietilpināšana**, lai katras kolonnas platums tiktu automātiski koriģēts, lai ietilpinātu saturu.

#### <a name="specify-the-width-of-a-column-on-a-report"></a>Kolonnas platuma norādīšana pārskatā

1. Pārskatu veidotājā atveriet modificējamās kolonnas definīciju.
2.  **Kolonnas platuma** šūnā ievadiet kolonnas platuma atstarpju skaitu. Katras kolonnas maksimālais platums 255 rakstzīmes (šis skaitlis ietver centus, komatus un iekavas). Vai arī, lai iespējotu pārskatu veidotājam atlasīt kolonnas platumu, pamatojoties uz šūnas saturu, **veiciet** dubultklikšķi uz kolonnas platuma šūnas un pēc tam noklikšķiniet **uz Automātiskā ietilpināšana**.

### <a name="add-space-between-columns"></a>Atstarpes starp kolonnām pievienošana

Papildu **atstarpes pirms kolonnas** šūnas norāda atdalītāja platumu starp vienu kolonnu un blakus esošām kolonnām kolonnas definīcijā. Papildu **atstarpes pirms kolonnas iestatījuma** ietekmē visas kolonnas detalizētās rindas kolonnai, bet ne kolonnas virsraksta rindas. Izmantojiet šo opciju, lai atdalītu kolonnu grupas vai pievienotu dažas atstarpes pirms apraksta, tādējādi atdalot apraksta kolonnu no kreisajā pusē līdzinātajiem pārskata virsrakstiem. Pēc noklusējuma starp kolonnām tiek ievietotas divas atstarpes. Šo iestatījumu var mainīt cilnē **Iestatījumi** pārskata definīcijā.

#### <a name="specify-the-space-between-columns"></a>Starp kolonnām esošās atstarpes norādīšana

1. Pārskatu veidotājā atveriet modificējamās kolonnas definīciju.
2. Papildu atstarpēs **pirms kolonnas šūnas** ievadiet atstarpju skaitu, ko ievietot starp kolonnām.

### <a name="specify-a-format-currency-override"></a>Formāta vai valūtas ignorēšanas norādīšana

Formāta **/valūtas ignorēšanas** šūna norāda decimāldaļas, valūtas un procentu summas formatēšanu kolonnā. Šis formatējums aizstāj formatējumu, kas ir norādīts pārskata definīcijā vai sistēmas noklusējuma iestatījumos.
...
1. Pārskatu veidotājā atveriet modificējamās kolonnas definīciju.
2. Veiciet dubultklikšķi summas **kolonnas formāta/valūtas** ignorēšanas šūnā.
...
Drukāšanas **kontroles** šūna var ietvert kodus, kas pielāgo displeju vai kolonnas drukāšanas raksturlielumus. Ir divu veidu drukas vadības kodi: parastie drukas vadības kodi un nosacījuma drukas vadības kodi.
...
1. Pārskatu veidotājā atveriet modificējamās kolonnas definīciju.
2. Veiciet dubultklikšķi uz vadīklas **drukāšanas** šūnas.
3. Dialoglodziņā Drukāt **kontroles** atlasiet kodu sarakstā Atlasīt drukāšanas **kontroles** opcijas. Lai atlasītu vairāk nekā vienu kodu, turiet nospiestu taustiņu Ctrl un atlasiet kodus.
...
> [!TIP]
> Drukāšanas kodus var ievadīt arī tieši drukāšanas **kontroles** šūnā. Vairāku drukas kodu atdalīšanai izmantojiet komatu.
...
Tālāk minētās **kolonnas definīcijas** rindas definīcijas attiecas uz kolonnām, kurām ir FD kolonnas **tips**  (summas no finanšu dimensijām).

#### <a name="book-codeattribute-category-cell"></a>Šūna Uzskaites kods/atribūtu kategorija

Kategorijas **šūna Grāmata ar kodu** /atribūtu identificē grāmatas kodu datiem **FD** kolonnā. Kolonnas definīcijā var ietvert vairākas faktisko, budžeta un statistikas vērtību kolonnas. Kolonnas definīcijā var tikt parādīti arī atšķirīgi periodi, piemēram, pašreizējais vai no gada sākuma, kā arī atšķirīgas summas. Grāmatas kodu saraksts atspoguļo faktiskās, budžeta un statistikas (nefinanšu) opcijas, kas ir izveidotas jūsu finanšu datos.

#### <a name="fiscal-year-cell"></a>Finanšu gada šūna

Finanšu **gada šūna** identificē finanšu gadu, kurā jāiekļauj kolonna. Gads var būt saistīts ar to pamata gadu, kas tika norādīts pārskata ģenerēšanas laikā. Ir pieejamas tālāk minētās opcijas.
...
#### <a name="periods-covered-cell"></a>Šūnas segtie periodi

Šūna **Periodi** sedza summu, kas jārāda kolonnā. Šī summa ir attiecībā pret kolonnas finanšu **gada** un **perioda** šūnu vērtību. Ir pieejamas tālāk minētās opcijas.
...
1. Pārskatu veidotājā atveriet modificējamās kolonnas definīciju.
2.  **FD kolonnā** veiciet dubultklikšķi uz šūnas rindā Nosegti **periodi** un sarakstā atlasiet kādu opciju.

### <a name="attribute-filter-in-a-column-definition"></a>Atribūtu filtrs kolonnas definīcijā

Atribūti ir finanšu datu vērtības, ko izmanto konta vai darījuma definēšanai. Konta atribūti ietver **Pamatlīdzeklis**, **Saistības**, **Ieņēmumi** un **Izdevumi**. Darbības atribūti ietver darbības aprakstu **un** darījuma **spēkā stāšanās datumu**. Atribūta atbalsts var atšķirties no Microsoft Dynamics 365 Finan finansu. Atribūta **filtra** šūna FD kolonnās ierobežo datus **līdz** noteiktām vērtībām vai atribūtu kategoriju diapazoniem. Kaut gan šo līdzekli var izmantot kopā ar **ATTR** kolonnu, **ATTR** kolonna nav nepieciešama. Kolonnā **FD** ir ierobežojums kontiem vai darbībām, kurus pārskats iekļaus no atribūta filtra.
...
1. Pārskatu veidotājā atveriet modificējamās kolonnas definīciju.
2. Veiciet dubultklikšķi FD **kolonnas** atribūtu filtra **šūnā**.
3.  **Atribūta filtra dialoglodziņā** veiciet dubultklikšķi uz šūnas atribūta kolonnā **un** pēc tam atlasiet filtra tipu.
...
Tālāk sniegtajā piemērā ir parādīta kolonnas apraksta daļa, kuras konta atribūts ir rindā **Grāmatas kods/Atribūts** . Šīs kolonnas atribūtu filtrs norāda vērtību diapazonu, kas jāietver pārskatā.
...
1. Pārskatu veidotājā atveriet modificējamās kolonnas definīciju.
2. Veiciet dubultklikšķi FD **kolonnas** **dimensiju filtra** šūnā.
...
- **Valūtas rādījums**  – norādiet valūtas tipu (uzskaite, pārskats, darbība vai tulkots pārskats), kurā darbības tiek rādītas. Pārrēķināšanas uz pārskata valūtu funkcionalitāte dažreiz tiek dēvēta par valūtas pārrēķināšanu. Valūtas pārrēķināšana nodrošina iespēju pārskatā ietvert virsgrāmatas summas, kas ir norādītas citā valūtā nekā uzņēmuma funkcionālā vai pārskata valūtā vai valūtā, kurā ievadījāt darījumu.
...
3.  **Dialoglodziņā Skatīt uzņēmuma** dialoglodziņu, sadaļā **Reģionālās opcijas** varat skatīt valūtu, kas noteikta atlasītajam uzņēmumam.

#### <a name="specify-the-currency-on-a-multiple-currency-report"></a>Valūtas norādīšana daudzvalūtu pārskatā

1. Pārskatu veidotājā atveriet modificējamās kolonnas definīciju.
2. Veiciet dubultklikšķi **uz**  **valūtas maiņas parādīšanas šūnas attiecīgajā FD** kolonnā un pēc tam atlasiet opciju valūtas informācijas parādīšanai: **Virsgrāmatas** uzskaites valūta, **Virsgrāmatas** pārskats, darbības valūta vai atlasiet, lai tulkotu citā pārskata valūtā.
...
### <a name="example-for-currency-display-and-currency-filter-cells"></a>Piemērs valūtas rādīšanai un valūtas filtra šūnām

Lietotājs atlasīja šādu valūtu kolonnas definīcijā:

- **Valūtas filtrs:** Yen
- **Rādītā valūta:**  Virsgrāmatas uzskaites valūta (ASV dolāri)
...
#### <a name="currency-filter-and-currency-display-combinations"></a>Valūtas filtra un valūtas rādīšanas kombinācijas

Tabulā ir parādīti pārskata rezultāti, kas var rasties dažādām opciju kombinācijām Valūtas displejā un Valūtas filtra **šūnās**  **veikto** atlases dēļ. Funkcionālā valūta ir USD.
...
| Virsgrāmatas uzskaites valūta | **JĒNA**              |**$60** – rezultāts rāda tikai darbības, kas tika ievadītas JPY, un attēlo tās darbības USD.<p><strong>Piezīme:</strong> konvertēšanas maiņas kurss ir aptuveni 100 JPY par vienu USD.</p> |
| Virsgrāmatas uzskaites valūta | Tukšs                | **$2310** — rezultāts rāda visus datus uzskaites valūtā, kas ir norādīta virsgrāmatā.<p><strong>Piezīme.</strong> Šī summa ir visu darījumu summa uzskaites valūtā.</p> |
