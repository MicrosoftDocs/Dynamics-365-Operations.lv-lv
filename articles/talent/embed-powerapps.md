---
title: "PowerApps pr. iegulš. Core HR"
description: "Šajā tēmā izskaidrots, kā atrisināt problēmu, kad PowerApps izvēlnes elements netiek rādīts sistēmas administrēšanas modulī."
author: Darinkramer
manager: AnnBe
ms.date: 11/02/2018
ms.topic: article
ms.prod: 
ms.service: dynamics-365-talent
ms.technology: 
audience: Application User
ms.reviewer: josaw
ms.search.scope: Talent
ms.custom: 
ms.assetid: 
ms.search.region: Global
ms.author: dkrame
ms.search.validFrom: 2018-11-02
ms.dyn365.ops.version: Talent
ms.translationtype: HT
ms.sourcegitcommit: d3f974f94b6c327fd70b8098d24f9e1f1e1e8eeb
ms.openlocfilehash: 197b553f0b202ee29ad42274e2c0e03446ec782c
ms.contentlocale: lv-lv
ms.lasthandoff: 12/04/2018

---

# <a name="embed-powerapps-apps-in-core-hr"></a>PowerApps pr. iegulš. Core HR

[!include [banner](includes/banner.md)]

**Izsniegt**

**PowerApps** izvēlnes elements netiek rādīts modulī **Sistēmas administrēšana**.

**Iemesls**

Lietot. interfeisa (UI) dizains ir mainīts, un Microsoft PowerApps programmas tagad ir iekļautas standarta personaliz. modelī.

**Izšķirtspēja**

Ir mainīts veids, kā PowerApps programmas tiek iegultas. PowerApps progr. tagad pievieno, lietojot personal. modeli. PowerApps progr. var piev. gandrīz visās Microsoft Dynamics 365 for Talent lapās.

Inf. par to, kā programmā Talent iegult PowerApps progr., sk. [PowerApps progr. ieg](https://docs.microsoft.com/en-us/dynamics365/unified-operations/fin-and-ops/get-started/embed-power-apps).

Visiem PowerApps klientiem, kas veica progr. iegulš. pirms izm., ir jābūt jauninātiem uz jauno modeli.

Poga **PowerApps** ir gandrīz visu Talent lapu augšējā labajā stūrī. Šo pogu var lietot, lai ievietotu PowerApps progr.

Tālāk ir minēts piemērs.

1. Dod. uz **Person. pārvald. \> Saites \> Nodarbin. \> Darbin**.
2. Atlasiet pogu **PowerApps** un tad atlasiet **Ievietot PowerApp**.

    ![PowerApps poga](media/png.png)

3. Aizpildiet laukus dialoglodziņā **Ievietot PowerApp**.

    ![Dialogl. Ievietot PowerApp](media/insert-powerapp.png)

Vai arī veiciet šādas darbības.

1. Lapas darbību rūts cilnes **Opcijas** grupā **Personalizēt** atlasiet **Personalizēt šo veidlapu**.

    ![Personalizēt grupu cilnē Opcijas](media/options.png)

    Tiek parādīta personaliz. rīkjosla.

2. Rīkjoslā atlasiet **Ievietot \> PowerApp**.

    ![PowerApps progr. ievietošana, lietojot personaliz. rīkjoslu](media/powerapp-bar.png)

