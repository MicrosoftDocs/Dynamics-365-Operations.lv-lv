...
Šis paraugs sastāv no Commerce Runtime (CRT), Retail Server un POS paplašinājumiem. Lai palaistu šo paraugu, ir jāmodificē un jāveido CRT, Retail Server un POS projekti. Ieteicams izmantot nemodificētu komplektu Retail SDK, lai veiktu šajā rakstā aprakstītās izmaiņas. Iesakām arī lietot avota kontroles sistēmu, piemēram, Microsoft Visual Studio Online (VSO), kur vēl nav mainīts neviens fails.

> [!NOTE]
> Programmā Commerce 10.0.8 un iepriekš lietojumprogrammā Retail Server ir zināms kā Commerce Scale Unit. Tā kā šis raksts attiecas uz vairākām iepriekšējām programmas versijām, *visā rakstā tiek izmantots Retail Server*.
>
> Daži šī raksta procedūru soļi atšķiras atkarībā no izmantotās Commerce versijas. Papildinformāciju skatiet sadaļā [Jaunumi vai izmaiņas Dynamics 365 Retail](../get-started/whats-new.md).

### <a name="using-certificate-profiles-in-commerce-channels"></a>Sertifikātu profilu izmantošana Commerce kanālos

Commerce versijās 10.0.15 un jaunākās versijās varat izmantot lietotāja definētos mazumtirdzniecības veikalu līdzekļu sertifikātu profilus, kas atbalsta kļūmes bezsaistes režīmā, [kad](./certificate-profiles-for-retail-stores.md) nav pieejama atslēga Outlook vai Commerce Headquarters. Šī funkcija paplašina [mazumtirdzniecības kanālu funkcijas pārvaldīt slepus](../dev-itpro/manage-secrets.md).

Lai lietotu šo funkcionalitāti paplašinājumā CRT, sekojiet šiem soļiem.

1. Izveidot jaunu paplašinājuma CRT projektu (C# klases bibliotēkas projekta tips). Izmantojiet mazumtirdzniecības programmatūras izstrādes komplekta (SDK) (RetailSDK\SampleExtensions\CommerceRuntime) parauga veidnes.
...
3. Lai lasītu slepeno izsaukumu, `GetUserDefinedSecretCertificateServiceRequest` izmantojot konstruktoru ar profila ID parametru. Tas sāks funkcionalitāti, strādājot ar iestatījumiem no sertifikātu profiliem. Atbilstoši iestatījumiem sertifikāts tiks izgūts no Azure atslēgas Lomas vai lokālās mašīnas krātuves.
...
5. Izveidojiet paplašinājuma CRT projektu.
...
Paplašinājuma CRT komponenti tiek iekļauti paraugos CRT. Lai izpildītu tālāk norādītās procedūras, atveriet CRT **risinājumu CommerceRuntimeSamples.sln**, **kas atrodas retailSdk\\ SampleExtensions\\ CommerceRuntime**.
...
3. Kopēt montāžas failu uz CRT paplašinājumu mapi:
    ...
    - **Lokāls CRT modernajā POS:** kopējiet montāžu uz **\\ ārējo** mapi lokālā klienta starpnieka CRT atrašanās vietā.

4. Meklēt paplašinājuma konfigurācijas failu šim CRT:

    - **Retail Server:** faila nosaukums ir **commerceruntime.ext.config**, un tā atrodas nodalījuma ārējā mapē, kas atrodas IIS Retail Server vietnes atrašanās **\\** vietā.
...
# <a name="application-update-4"></a>[Programmatūras atjauninājums 4](#tab/app-update-4)
...
6. Meklēt paplašinājuma konfigurācijas failu šim CRT:

    - **Retail Server:** faila nosaukums ir **commerceruntime.ext.config**, un tā atrodas nodalījuma ārējā mapē, kas atrodas IIS Retail Server vietnes atrašanās **\\** vietā.
    - **Modern CRT POS lokāls:** faila **nosaukums ir CommerceRuntime.MPOSOffline.Ext.config**, un tas atrodas vietējā klienta starpnieka atrašanās CRT vietā.

7. Reģistrēt izmaiņas CRT paplašinājuma konfigurācijas failā.
...
6. Reģistrējiet Retail Server paplašinājumu atkarības.
    ...
    3. Reģistrējiet CRT izmaiņas paplašinājuma konfigurācijas failā CRT. Šī faila nosaukums ir **commerceruntime.ext.config**, **un** tas atrodas nodalījuma mapē zem IIS Retail Server vietnes atrašanās vietas.
...
Šī daļa ir ekvivalenta Retail Servera kontrollerim, taču tā paplašina lokālu versiju, ko izmanto, kad klients nav izveidojis CRT savienojumu.
...
4. Atrisināt Proxies.RetailProxy **projekta** atkarības:
    ...
    Atveriet RetailSDK **Proxies\\ RetailProxy\\ Proxies.RetailProxy.csproj\\,** pievienojiet risinājumam RetailSDK **SampleExtensions\\ CommerceRuntime\\ Extensions.SalesTransactionSignmediaSample\\ CommerceRuntime.Extensions.SalesTransactionSignoxySample\\** projektu un pievienojiet projekta atsauci projektam RetailProxy **atsaucei** uz SalesTransactionSignactionSample **.**
...
3. Kopējiet komplektācijas failus ārējā **\\ mapē** lokālā klienta starpnieka CRT atrašanās vietā.
4. Reģistrēt mazumtirdzniecības starpniekservera izmaiņas paplašinājuma konfigurācijas failā. Faila nosaukums ir **RetailProxy.MPOSOffline.ext.config**, un tas atrodas lokālā klienta starpnieka CRT atrašanās vietā.
...
1. Atveriet risinājumu, kas **atrodas RetailSdk\\ POS\\ ModernPOS.sln**, un pārliecinieties, vai to var kompilēt bez kļūdām. Turklāt pārliecinieties, ka modern POS var darbināt no Microsoft Visual Studio, izmantojot komandu **Palaist**.
...
1. Veiciet darbības, kas ir jāveic [tālāk šī raksta](#cloud-pos-extension-components)[sadaļā Mākoņa POS paplašinājuma komponentos](#modern-pos-extension-components) vai Modern POS paplašinājuma komponentu sadaļā.
...
3. Pielāgošanas.iestatījumu pakotnes pielāgošanas **konfigurācijas** failā veiciet šādas izmaiņas:
    ...
    2. Pievienojiet sadaļai ItemGroup **tālāk** norādītās rindas, lai ietvertu paplašinājumus CRT izvietojamās pakotnēs:
...
8. Izmantojiet pakotnes, izmantojot Microsoft Dynamics Lifecycle Services (LCS) vai manuāli. Papildinformāciju skatiet sadaļā [Izvietojamu pakotņu izveide](../dev-itpro/retail-sdk/retail-sdk-packaging.md).
