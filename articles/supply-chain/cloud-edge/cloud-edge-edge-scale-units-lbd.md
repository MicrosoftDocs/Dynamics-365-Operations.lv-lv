...
Malas apjoma vienības tiek izpildītas lokāli izmantotajām vidēm, tāpēc infrastruktūras prasības ir ļoti līdzīgas. Tomēr ir noteiktas atšķirības, kas jānorāda:

- Malu skalas vienības neizmanto finanšu pārskatus, tādēļ tām nav nepieciešami finanšu pārskatu mezgli.
- Ražošanas un noliktavas darba noslodzes nav skaitļošanas intensīva, tāpēc apsveriet iespēju atbilstīgi mainīt skaitļošanas jaudu AOS zariem.
...
1. Pēc datu bāzu konfigurēšanas [soļa izpildes](../../fin-ops-core/dev-itpro/deployment/setup-deploy-on-premises-pu41.md#configuredb) izpildiet tālāk norādīto skriptu, lai konfigurētu Mēroga vienības Instrument Instrumenttor datu bāzi.
...
1. Pēc TAM, kad esat pabeidzis [AD FS konfigurēšanas](../../fin-ops-core/dev-itpro/deployment/setup-deploy-on-premises-pu41.md#configuredb) darbību, sekojiet šiem soļiem:
    ...
    1. Izveidot jaunu Azure Active Directory () programmu Azure AD, kas iespējos Neplānotās instrumentācijas pakalpojumu komunicēt ar Mēroga vienību pārvaldības pakalpojumu.
...
1. Pēc vides izvietošanas izpildiet tālāk minētās darbības.
    ...
    1. Pievienojiet iepriekš izveidoto AD FS programmas ID (izmantojot skriptu Create-ADFSServerApplicationForEdgeScaleUnits.ps1) Azure AD apjoma mērvienību lietojumprogrammu tabulai. Šo darbību var manuāli pabeigt, izmantojot lietotāja interfeisu (UI). Alternatīvi to var pabeigt, izmantojot datu bāzi, izmantojot šādu skriptu.
...
## <a name="set-up-an-azure-key-vault-and-an-azure-ad-application-to-enable-communication-between-scale-units"></a><a name="set-up-keyvault"></a> Iestatīt Azure atslēgu Windows un programmu, lai Azure AD iespējotu sakarus starp mēroga vienībām

1. Pēc vides izvietošanas izveidojiet papildu programmu, lai iespējotu Azure AD uzticamus sakarus starp pārkraušanas punktu un mēroga vienību.
...
1. Pēc programmas izveides ir jāizveido klienta noslēpums un jāsaglabā informācija Azure atslēgā. Turklāt jums ir jāpiešķir piekļuve Azure AD izveidotajām programmai, lai varētu izgūt noslēpumus, kas tiek glabāti atslēgas krātuvē. Jūsu ērtību gadījumā šis skripts automātiski izpildīs visas nepieciešamās darbības.
...
1. Pievienojiet tikko Azure AD izveidoto programmas ID (izmantojot skriptu Create-PpabAADApplication.ps1) Azure AD programmu tabulai jūsu pārkraušanas punktā. Šo darbību varat manuāli pabeigt, izmantojot UI.
...
Jūs konfigurējat un pārvaldāt savu malu skalas vienību, izmantojot Mēroga vienību pārvaldības portālu. Papildinformāciju skatiet mēroga [vienību un darba noslodzi pārvaldība, izmantojot mēroga vienību pārvaldnieka portālu](./cloud-edge-landing-page.md#scale-unit-manager-portal).
