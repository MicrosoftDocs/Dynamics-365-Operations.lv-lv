...
Krājumu **vērtības pārskata glabāšanas pārskats nodrošina** izvadi kā interaktīvu lapu programmā Microsoft vai kā Dynamics 365 Supply Chain Management eksportētu dokumentu vienā no vairākiem formātiem.
...
## <a name="turn-on-the-inventory-value-report-storage-feature"></a>Ieslēgt krājumu vērtības pārskata glabāšanas līdzekli

Standarta krājumu **vērtības pārskats** tiek aktivizēts pēc noklusējuma. Tomēr, ja vēlaties izveidot detalizētāku krājumu **vērtības pārskata glabāšanas** pārskatu, ir jāieslēdz funkcija sistēmā. Administratori var izmantot [funkciju pārvaldības](../../fin-ops-core/fin-ops/get-started/feature-management/feature-management-overview.md) iestatījumus, lai pārbaudītu līdzekļa statusu un to ieslēgtu, ja nepieciešams. Darbvietā **Līdzekļu pārvaldība** šis līdzeklis ir uzskaitīts šādi:

- **Modulis:** *izmaksu pārvaldība*
- **Līdzekļu nosaukums:** *krājumu vērtības pārskata glabāšana*
...
1. Kopsavilkuma cilnē **Vispārīgi**, iestatiet tālāk minētos laukus:
    ...
    - **Dimensiju** kopa – izvēlieties dimensiju kopu, kuras datus palaist. (Dimensijas ir definētas Virsgrāmatā.) Piemēram, varat darbināt datus galvenajam *kontam vai* galvenajam kontam + biznesa *·* vienībai. Jūsu atlasāmā dimensiju kopa nedrīkst būt lielāka par divām dimensijām. Papildinformāciju skatiet finanšu [dimensiju](../../finance/general-ledger/financial-dimension-sets.md) kopās.
...
1. Kopsavilkuma cilnē **·** Kolonnas iestatiet šādus laukus. Šie lauki kontrolē kolonnas, ko ietver jūsu pārskats, un šo kolonnu datu tipus.
    ...
    - **Aprēķināt vidējās vienības izmaksas** – iestatiet šo opciju kā *Jā, lai parādītu* kolonnu, kas parāda vidējās vienības izmaksas. Vidējās vienības izmaksas ir kopējais daudzums, kas dalīts ar kopsummu.
    ...
    - **Resursa ID** – iestatiet opciju Skats uz **·** *Jā, lai parādītu* kolonnu, kas identificē krājumu katrai rindai. Lai iekļautu **·** kopsummas, *iestatiet opciju* Kopsumma. Atkarībā no katrā rindā uzskaitītā krājuma tipa kolonna rāda vienu no šiem informācijas tipiem:
        ...
        - **Materiāls** – kolonnā parādīta lauka vērtība `ItemID` atbilstošajam materiāla ierakstam.
        - **Darbaspēks** – kolonnā parādīta lauka vērtība `WorkCenterID` atbilstošam darba ierakstam.
        - **Netiešās** izmaksas – kolonna rāda lauka vērtību `CodeID` atbilstošam izmaksu ierakstam.
    ...
    - **Resursu grupa** - iestatiet opciju Skats uz **·** *Jā, lai parādītu* kolonnu, kas identificē resursu grupu katrai rindai. Lai iekļautu **·** kopsummas, *iestatiet opciju* Kopsumma. Atkarībā no katrā rindā uzskaitītā krājuma tipa kolonna rāda vienu no šiem informācijas tipiem:
        ...
        - **Materiāls** – kolonnā parādīta lauka vērtība `ItemGroup` atbilstošajam materiāla ierakstam.
        - **Darbaspēks** – kolonnā parādīta lauka vērtība `WorkcenterGroup` atbilstošam darba ierakstam.
        - **Netiešās** izmaksas – kolonna rāda lauka vērtību `CostGroup` atbilstošam izmaksu ierakstam. `CostGroupType`(Vērtībai jābūt *·* Netiešai.)
...
1. Kopsavilkuma **cilnes Parametri** dialoglodziņā Krājumu vērtība iestatiet šādus **·** laukus:
    ...
    - **ID** – izvēlieties krājumu vērtības pārskata [...](#report-configuration) konfigurāciju, ko izmantot pārskatam. Konfigurācija izveido opcijas kolonnām un rindām, kas tiks iekļautas pārskatā.
...
1. Dodieties uz **Izmaksu pārvaldība \> Vaicājumi un pārskati \> Krājuma vērtības pārskata uzglabāšana**.
1. Sarakstā atlasiet pārskatu. Lapā ir parādīta detalizēta informācija par krājumu [vērtības pārskata](#report-configuration) konfigurāciju, kas tika izmantota, lai ģenerētu atlasīto pārskatu.
