...
Finanšu ieskati apvieno Microsoft Dynamics 365 Finance funkcionalitāti ar Common Data Service, Azure un AI Builder, lai nodrošinātu jaudīgus prognozēšanas rīkus jūsu organizācijai. Šajā tēmā ir izskaidrotas konfigurācijas darbības, kas ļaus jūsu sistēmai izmantot iespējas, kas pieejamas Finanšu ieskatos.
...
## <a name="configure-common-data-service"></a>Common Data Service konfigurēšana
...
1. Atveriet [Power Platform administrēšanas centru](https://admin.powerplatform.microsoft.com/) un veiciet tālāk norādītās darbības, lai izveidotu jaunu Common Data Service vidi tajā pašā Active Directory nomniekā.
    ...
    11. Pierakstiet Common Data Service organizācijas ID.
    ...
    16. Iestatiet laukā **Instances atsauces informācijas ID** Common Data Service organizācijas ID vērtību, ko pierakstījāt iepriekš.
    17. Pierakstiet pārlūkprogrammas adreses joslā esošo Common Data Service organizācijas URL. Piemēram, URL varētu būt `https://org42b2b3d3.crm.dynamics.com`.
...
### <a name="enter-the-common-data-service-directory-id-and-the-users-azure-ad-object-id"></a>Ievadiet Common Data Service direktorijas ID un lietotāja Azure AD objekta ID

1. Ievadiet Common Data Service direktorijas ID.

    1. Atveriet [Azure portālu](https://portal.azure.com).
    2. Pierakstieties, izmantojot lietotāja ID, kas tika izmantots Common Data Service vides izveidei.
...
> [!NOTE]
> Pārliecinieties, ka izveidojat tālāk norādītos resursus tajā pašā Azure AD instancē kā Common Data Service vide. Nav iespējams izmantot resursus no citas Azure AD instances.
...
4. Pievienojumprogrammas detalizētās informācijas lapā **Iegūt ieskatus** ievadiet tālāk norādītās vērtības.
    ...
    | Vērtība                                                    | Apraksts |
    |----------------------------------------------------------|-------------|
    | CDS organizācijas URL                                     | Common Data Service organizācijas URL Common Data Service instancē. Lai atrastu šo vērtību, atveriet [Power Apps portālu](https://make.powerapps.com), atlasiet pogu **Iestatījumi** (zobrata simbols) augšējā labajā stūrī, atlasiet **Papildu iestatījumi** un kopējiet URL. (URL beidzas ar "dynamics.com.") |
    | CDS org. ID                                               | Common Data Service instances vides ID. Lai atrastu šo vērtību, atveriet [Power Apps portālu](https://make.powerapps.com), atlasiet pogu **Iestatījumi** (zobrata simbols) augšējā labajā stūrī, atlasiet **Pielāgojumi \> Izstrādātāja resursi \> Instances atsauces informācija** un kopējiet vērtību **ID**. |
    | CDS nomnieka ID (direktorija ID no AAD)               | Common Data Service instances nomnieka ID. Lai atrastu šo vērtību, atveriet [Azure portālu](https://portal.azure.com), dodieties uz **Azure Active Directory** un kopējiet vērtību **Nomnieka ID**. |
    | Norādiet tā lietotāja objekta ID, kam ir sistēmas administratora loma | Azure AD lietotāja objekta ID lietotājam Common Data Service. Šim lietotājam jābūt Common Data Service instances sistēmas administratoram. Lai atrastu šo vērtību, atveriet [Azure portālu](https://portal.azure.com), dodieties uz **Azure Active Directory \> Lietotāji**, atlasiet lietotāju un pēc tam sekcijā **Identitāte** kopējiet vērtību **Objekta ID**. |
    | Vai šī ir nomnieka noklusējuma CDS vide?      | Ja Common Data Service instance bija pirmā izveidotā ražošanas instance, atzīmējiet šo izvēles rūtiņu. Ja Common Data Service instance tika izveidota manuāli, izņemiet atzīmi no šīs izvēles rūtiņas. |
