...
> [!NOTE]
> * Definējiet atslēgu, kas pieprasījuma veidlapā tiek izmantota kā `private const str`.
> * Iestatiet metodes `copyToTaxableDocumentLineWrapperFromTaxIntegrationLineObjectByLine` lauku, izmantojot `SetField` metodi. Otrā parametra datu tipam jābūt `string`. Ja datu tips nav `string`, konvertējiet to par `string`.

## <a name="appendix"></a>Pielikums

Šajā pielikumā ir parādīts pilns parauga kods finanšu dimensiju integrēšanai (**Izmaksu centrs** un **Projekts**) rindas līmenī.
